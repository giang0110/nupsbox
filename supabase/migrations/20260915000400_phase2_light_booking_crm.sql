-- Phase 2 P2.3: lightweight viewing appointments for the lead CRM.
-- This does not reserve inventory, decrement availability, or take payment.
...
create table public.lead_appointments (
  ...
  duration_minutes integer not null default 30
    constraint lead_appointments_duration_check check (duration_minutes between 15 and 180),
...
create table public.lead_appointment_history (
  ...
  event_type text not null
    constraint lead_appointment_history_event_type_check check (event_type in (
      'created',
      'rescheduled',
      'location_changed',
      'unit_type_changed',
...
begin
  if tg_op = 'INSERT' and new.status <> 'pending' then
    raise exception 'new appointment must start pending'
      using errcode = '23514';
  end if;

  if tg_op = 'INSERT' and auth.uid() is not null and new.source <> 'staff' then
    raise exception 'authenticated appointment must use staff source'
      using errcode = '23514';
  end if;
...
declare
  ...
  v_after jsonb;
begin
  v_after := public.appointment_snapshot(new);
  ...
  v_before := public.appointment_snapshot(old);

  if old.status is distinct from new.status then
    insert into public.lead_appointment_history (
      appointment_id, lead_id, changed_by, event_type, before_state, after_state
    ) values (
      new.id, new.lead_id, auth.uid(), 'status_changed', v_before, v_after
    );
  end if;
  ...
  if old.scheduled_at is distinct from new.scheduled_at then
    insert into public.lead_appointment_history (
      ...
      new.id, new.lead_id, auth.uid(), 'rescheduled', v_before, v_after
    );
  end if;
  ...
  if old.location_id is distinct from new.location_id then
    insert into public.lead_appointment_history (
      ...
      new.id, new.lead_id, auth.uid(), 'location_changed', v_before, v_after
    );
  end if;
  ...
  if old.unit_type_id is distinct from new.unit_type_id then
    insert into public.lead_appointment_history (
      ...
      new.id, new.lead_id, auth.uid(), 'unit_type_changed', v_before, v_after
    );
  end if;
  ...
  if old.assigned_to is distinct from new.assigned_to then
    insert into public.lead_appointment_history (
      ...
      new.id, new.lead_id, auth.uid(), 'assignee_changed', v_before, v_after
    );
  end if;
  ...
  if old.duration_minutes is distinct from new.duration_minutes
    ...
    insert into public.lead_appointment_history (
      ...
      new.id, new.lead_id, auth.uid(), 'details_changed', v_before, v_after
    );
  end if;
...
-- Appointment viewers need to render the active staff/admin assignee name.
