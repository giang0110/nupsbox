...
select policies_are('public', 'content_blocks', array['content_blocks_authenticated_read_all', 'content_blocks_public_read'], 'content blocks remain internal read-only for authenticated users');

select policy_roles_are('public', 'locations', 'locations_public_read', array['anon', 'authenticated'], 'catalog is publicly readable');
select policy_roles_are('public', 'locations', 'locations_authenticated_read_all', array['authenticated'], 'internal location read is authenticated only');
select policy_roles_are('public', 'content_blocks', 'content_blocks_authenticated_read_all', array['authenticated'], 'internal content block read is authenticated only');
