...
export async function MobileActionBar({phoneUrl, zaloUrl, mode = 'default'}: MobileActionBarProps) {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-3 bottom-[max(.75rem,env(safe-area-inset-bottom))] z-[60] grid grid-cols-2 gap-1.5 rounded-2xl border border-black/10 bg-white/96 p-1.5 shadow-[0_16px_50px_rgba(7,26,56,0.24)] backdrop-blur-xl lg:hidden"
    >
      <ConversionCta
        locale={locale}
        intent="finder"
        ...
        size="md"
        className="min-h-14 rounded-xl px-3 text-xs"
      >
      ...
      {contactAction ? (
        <a
          ...
          className="col-span-2 flex min-h-11 items-center justify-center gap-2 rounded-xl text-xs font-semibold text-[var(--nupsbox-navy)] hover:bg-[var(--nupsbox-surface)]"
        >
