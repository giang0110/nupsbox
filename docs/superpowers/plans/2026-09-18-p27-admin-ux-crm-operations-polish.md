...
- [ ] **Step 3: Implement pure CRM aggregation**

Add to `features/admin/dashboard.ts`:
...
Keep `normalizeAdminDashboardCounts` for health counts so the existing regression remains valid.
...
const upcomingCount = await appointmentClient
...
- batch the unique appointment `lead_id` values into one `leads` identity query with `.in('id', leadIds)`;
- mark only past confirmed appointments as `overdue: true`;
- preserve bounded queue sizes;
- keep health count queries for active locations, active unit types, active FAQ, and published blog posts;
- return one `AdminDashboardSummary` object.

Do not compute a conversion rate in P2.7.
