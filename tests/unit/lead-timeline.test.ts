...
describe('unified CRM lead timeline', () => {
  it('combines lead and appointment history newest first', () => {
    const timeline = buildLeadTimeline({
      notes: [
        {
          id: 'n1', note: 'Called', authorName: 'Staff A',
          createdAt: '2026-09-15T02:00:00.000Z'
        }
      ],
      statusHistory: [
        {
          id: 's1', fromStatus: 'new', toStatus: 'contacted', changedByName: 'Staff A',
    ...
    expect(timeline.map((item) => item.kind)).toEqual(['appointment', 'lead_status', 'note']);
  });
